...
# center with different appearance (uses block padding)
center() {
  termwidth=$(stty size | cut -d" " -f2)
  # use a block character for padding to change appearance
  padding="$(printf '%0.1s' ▁{1..500})"
  local msg="$1"
  # compute left/right width carefully (avoid negative width)
  local left_width=$(((termwidth-2-${#msg})/2))
  local right_width=$((termwidth-1-${#msg}-left_width))
  if [ $left_width -lt 0 ]; then left_width=0; fi
  if [ $right_width -lt 0 ]; then right_width=0; fi
  printf '%*.*s %s %*.*s\n' 0 "$left_width" "$padding" "$msg" 0 "$right_width" "$padding"
}

clear
center_banner

# Loading spinner (decoded from your base64)
spinner=( '|' '/' '-' "\\" )
...
# show loading using the new center style
center "Loading..."
count
